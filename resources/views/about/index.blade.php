...
<div class="row pt-4 mt-4">
    <div class="col-12 px-4">
        ...
        <div class="d-flex gap-4">
            <a class="btn btn-sm btn-light" href="{{url('/home')}}">Regresar</a>
            <a href="{{ route('abouts.create') }}" class="btn btn-dark btn-sm">Crear información</a>
        </div>
    </div>
</div>
                    ...
                    <table class="table table-borderless table-hover">
                        ...
                        <tbody>
                            ...
                            <tr>
                                ...
                                <td>{{ $about->title }}</td>
                                <td>{!! nl2br((Str::limit($about->body, 80)))!!}</td>

                                <td>
                                    <div class="d-flex justify-content-around align-items-center gap-2">
                                        @can('abouts.show')
                                        <a class="btn btn-sm btn-light" href="{{ route('abouts.show',$about->id) }}"><i
                                                class="bi bi-eye-fill fs-6"></i></a>
                                        @endcan
                                        @can('abouts.edit')
                                        <a class="btn btn-sm btn-dark" href="{{ route('abouts.edit',$about->id) }}"><i
                                                class="bi bi-pencil-fill fs-6"></i></a>
                                        @endcan
                                        @can('abouts.destroy')
                                        <button title="Eliminar" data-bs-toggle="modal"
                                            data-bs-target="#modal-delete-{{$about->id}}" type="submit"
                            ...
                            @can('abouts.destroy')
                            <!-- Modal -->
                            <div class="modal fade" id="modal-delete-{{$about->id}}"
                                data-bs-backdrop="modal-delete-{{$about->id}}" data-bs-keyboard="false" tabindex="-1"
                                aria-labelledby="modal-delete-{{$about->id}}" aria-hidden="true">
